...
// Import crossword helpers and handlers
const crosswordHelpers = require("./crosswordHelpers");
const { attachCrosswordRoutes } = require("./attachCrosswordRoutes");
const { attachCrosswordSocketHandlers } = require("./crosswordSocketHandlers");
...
// Attach crossword routes to main server
attachCrosswordRoutes(app, {
  pool,
  crosswordSessions,
  crosswordGameStatus,
  ...
  // Helpers
  generateCrosswordSessionId: crosswordHelpers.generateCrosswordSessionId,
  generateShortGameCode: crosswordHelpers.generateShortGameCode,
  normalizeCrosswordQuestions: crosswordHelpers.normalizeCrosswordQuestions,
  emitCrosswordGrid: crosswordHelpers.emitCrosswordGrid,
  emitCrosswordLeaderboard: (gameCode, sessionId) => 
    crosswordHelpers.emitCrosswordLeaderboard(gameCode, sessionId, io, crosswordGameStatus, pool),
  emitWaitingLeaderboard: (gameCode) =>
    crosswordHelpers.emitWaitingLeaderboard(gameCode, io, crosswordWaitingPlayers),
  getCrosswordWaitingLeaderboardRows: (gameCode) =>
    crosswordHelpers.getCrosswordWaitingLeaderboardRows(gameCode, crosswordWaitingPlayers),
  getCrosswordLeaderboardRows: (limit, sessionId) =>
    crosswordHelpers.getCrosswordLeaderboardRows(limit, sessionId, pool),
  getCrosswordAggregateLeaderboardRows: (limit) =>
    crosswordHelpers.getCrosswordAggregateLeaderboardRows(limit, pool),
  ensureCrosswordLeaderboardEntry: (userId, sessionId) =>
    crosswordHelpers.ensureCrosswordLeaderboardEntry(userId, sessionId, pool),
  getSolvedWordIdsForUser: (userId, sessionId) =>
    crosswordHelpers.getSolvedWordIdsForUser(userId, sessionId, pool),
  getOrCreateSolvedUserSet: crosswordHelpers.getOrCreateSolvedUserSet,
  getRemainingCrosswordTimeMs: crosswordHelpers.getRemainingCrosswordTimeMs,
  hasPlayableCrosswordSession: crosswordHelpers.hasPlayableCrosswordSession,
  getWaitingPlayerMap: (gameCode) =>
    crosswordHelpers.getWaitingPlayerMap(gameCode, crosswordWaitingPlayers),
  clearCrosswordTimer: (gameCode) =>
    crosswordHelpers.clearCrosswordTimer(gameCode, crosswordTimers),
  finalizeCrosswordGame: (gameCode, options) =>
    crosswordHelpers.finalizeCrosswordGame(gameCode, options, {
      crosswordGameStatus,
      crosswordSessions,
      crosswordTimers,
      crosswordWaitingPlayers,
      ...
      getCrosswordLeaderboardRows: (limit, sessionId) =>
        crosswordHelpers.getCrosswordLeaderboardRows(limit, sessionId, pool),
      getCrosswordWaitingLeaderboardRows: (gc) =>
        crosswordHelpers.getCrosswordWaitingLeaderboardRows(gc, crosswordWaitingPlayers),
      clearCrosswordTimer: (gameCode) =>
        crosswordHelpers.clearCrosswordTimer(gameCode, crosswordTimers),
    }),
  getDateFilter: crosswordHelpers.getDateFilter,
  io,
});
  ...
  try {
    ...
    const sql = `
      SELECT 
        ...
        COALESCE(SUM(CASE WHEN a.is_correct = 1 THEN 1 ELSE 0 END), 0) as correct_answers,
        CASE
          WHEN latest_session_count > 0 THEN ROUND((latest_session_correct * 100.0 / latest_session_count), 2)
          ELSE 0
        END as accuracy
      FROM users u
      LEFT JOIN answers a ON u.user_id = a.user_id
      LEFT JOIN (
        SELECT 
          user_id,
          SUM(CASE WHEN is_correct = 1 THEN 1 ELSE 0 END) as latest_session_correct,
          COUNT(*) as latest_session_count
        FROM answers
...
app.get("/student/:student_id/game-performance", async (req, res) => {
  try {
    ...
    // Get Wisdom Warfare performance - latest game session accuracy
        ...
        COALESCE(
          (SELECT 
            ...
            WHERE user_id = ? AND game_session_id = (
              SELECT game_session_id FROM answers WHERE user_id = ? ORDER BY answered_at DESC LIMIT 1
            )),
          0
        ) as accuracy
      FROM performance p
...
// Get difficulty breakdown for a specific game session
app.get("/user/:user_id/game-session/:game_session_id/difficulty-breakdown", async (req, res) => {
  try {
    const { user_id, game_session_id } = req.params;
    ...
    const [difficultyBreakdown] = await pool.query(
      ...
      FROM answers a
      JOIN questions q ON a.question_id = q.id
      WHERE a.user_id = ? AND a.game_session_id = ?
      GROUP BY q.difficulty
    `,
    ...
    const byDifficulty = {};
    difficultyBreakdown.forEach((stat) => {
      const difficultyKey = String(stat.difficulty).trim().toLowerCase();
      byDifficulty[difficultyKey] = {
        total: stat.total_answered || 0,
    ...
    ['easy', 'medium', 'hard'].forEach(level => {
      if (!byDifficulty[level]) {
        byDifficulty[level] = { total: 0, correct: 0, score: 0 };
      }
    });
    ...
    console.error("Get game session difficulty breakdown error:", err);
    res.status(500).json({ error: err.message });
  }
});
...
app.get("/teacher/:teacher_id/analytics", async (req, res) => {
  ...
  try {
    ...
    // Get average accuracy from answers table (filtered by timeRange) - Only Wisdom Warfare
    const [accuracyData] = await pool.query(
      "SELECT ROUND(SUM(CASE WHEN is_correct = 1 THEN 100 ELSE 0 END) / COUNT(*), 2) as avg FROM answers WHERE answered_at >= ?",
      [dateFilter]
    );
    const accuracy = [{ avg: accuracyData[0]?.avg || 0 }];
    ...
    // Current period accuracy - Only Wisdom Warfare
    const [currentAccData] = await pool.query(
      "SELECT ROUND(SUM(CASE WHEN is_correct = 1 THEN 100 ELSE 0 END) / NULLIF(COUNT(*), 0), 2) as avg FROM answers WHERE answered_at >= ?",
      [thirtyDaysAgo]
    );
    const [prevAccData] = await pool.query(
      "SELECT ROUND(SUM(CASE WHEN is_correct = 1 THEN 100 ELSE 0 END) / NULLIF(COUNT(*), 0), 2) as avg FROM answers WHERE answered_at BETWEEN ? AND ?",
      [sixtyDaysAgo, thirtyDaysAgo]
    );
    const currentAcc = [{ avg: currentAccData[0]?.avg || 0 }];
    const prevAcc = [{ avg: prevAccData[0]?.avg || 0 }];
...
// GET: Latest game session answers
app.get("/answers/latest-session", async (req, res) => {
  try {
    // Get the latest game_session_id that has answers
    const [sessionResult] = await pool.query(`
      SELECT game_session_id, MAX(answered_at) as latest_time
      FROM answers
      WHERE game_session_id IS NOT NULL AND game_session_id != ''
      GROUP BY game_session_id
    ...
    const latestGameSessionId = sessionResult[0].game_session_id;

    // Get all answers from that latest game session with topic info
    const [rows] = await pool.query(`
      SELECT a.*, q.text as question_text, q.correct as correct_answer, q.topic, q.difficulty
      FROM answers a
      JOIN questions q ON a.question_id = q.id
      WHERE a.game_session_id = ?
      ORDER BY a.answered_at DESC
    `, [latestGameSessionId]);
...
app.get("/teacher/analytics/question/:questionId/answers", async (req, res) => {
  ...
  try {
    ...
    // If type is 'latest-session', filter to only the latest session
    if (type === 'latest-session') {
      query += ` AND a.game_session_id = (
        SELECT game_session_id FROM answers ORDER BY answered_at DESC LIMIT 1
      )`;
    }
    
    query += ` ORDER BY a.answered_at DESC`;
    
    const [answers] = await pool.query(query, params);
...
app.get("/user/:userId/answers", async (req, res) => {
  ...
  try {
    ...
    } else if (type === 'latest-session') {
      // Get only answers from the latest game session for this user
      query = `
        SELECT 
          a.question_id,
          q.text as question_text,
...
// Attach crossword socket handlers
attachCrosswordSocketHandlers(io, {
  crosswordSessions,
  crosswordGameStatus,
  crosswordLocks,
  crosswordWaitingPlayers,
  ...
  getRemainingCrosswordTimeMs: crosswordHelpers.getRemainingCrosswordTimeMs,
  hasPlayableCrosswordSession: crosswordHelpers.hasPlayableCrosswordSession,
  upsertCrosswordScore: (userId, sessionId) =>
    crosswordHelpers.upsertCrosswordScore(userId, sessionId, pool),
  getWaitingPlayerMap: (gameCode) =>
    crosswordHelpers.getWaitingPlayerMap(gameCode, crosswordWaitingPlayers),
  clearCrosswordTimer: (gameCode) =>
    crosswordHelpers.clearCrosswordTimer(gameCode, crosswordTimers),
