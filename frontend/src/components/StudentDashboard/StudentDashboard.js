...
const StudentDashboard = () => {
  ...
  const [latestSessionDifficultyData, setLatestSessionDifficultyData] = useState({});
  ...
  useEffect(() => {
    console.log('Latest session difficulty data updated:', latestSessionDifficultyData);
  }, [latestSessionDifficultyData]);
  ...
  const fetchLatestSessionDifficultyBreakdown = async (gameSessionId) => {
    if (!gameSessionId) {
      console.warn('No game session ID provided');
      return;
    }
    try {
      console.log('Fetching difficulty breakdown for session:', gameSessionId);
      const response = await fetch(`${API_BASE}/user/${userId}/game-session/${gameSessionId}/difficulty-breakdown`);
      if (response.ok) {
        const data = await response.json();
        console.log('Difficulty breakdown data:', data);
        setLatestSessionDifficultyData(data.by_difficulty || {});
      } else {
        console.error('Failed to fetch difficulty breakdown:', response.status, response.statusText);
      }
    } catch (error) {
      console.error('Error fetching latest session difficulty breakdown:', error);
    }
  };
  ...
  const fetchUserStats = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUserStats(stats);
        const sessions = stats.game_sessions || [];
        console.log('Game sessions:', sessions);
        setGameSessions(sessions);
        
        // Fetch difficulty breakdown for latest session if available
        if (sessions.length > 0) {
          console.log('Fetching breakdown for latest session:', sessions[0]);
          fetchLatestSessionDifficultyBreakdown(sessions[0].game_session_id);
        } else {
          console.warn('No game sessions found');
        }
  ...
  // Use latest session difficulty data if available, otherwise use cumulative data
  const baseData = Object.keys(latestSessionDifficultyData).length > 0 
    ? latestSessionDifficultyData 
    : (gameSummary?.summary?.by_difficulty || {});
  
  // Ensure all difficulty levels have proper structure
  const difficultyData = {
    easy: baseData.easy || { total: 0, correct: 0, score: 0 },
    medium: baseData.medium || { total: 0, correct: 0, score: 0 },
    hard: baseData.hard || { total: 0, correct: 0, score: 0 }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-3 bg-gray-800 rounded-2xl p-6 border-2 border-green-600">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-green-300">📊 Score Breakdown by Difficulty</h2>
                {Object.keys(latestSessionDifficultyData).length > 0 && (
                  <span className="text-sm bg-green-700 text-green-200 px-3 py-1 rounded">Latest Session</span>
                )}
              </div>
              ...
              <div className="mt-4 text-center">
                <div className="text-lg text-gray-300">
                  Total Score: <span className="font-bold text-green-400">{gameSummary?.summary?.total_score || 0}</span>
                </div>
              </div>
            </div>
