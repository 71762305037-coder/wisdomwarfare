...
// ✅ Crossword Socket Configuration
// In PRODUCTION (Render): Connect to main server (both Socket.IO instances run there)
// In DEVELOPMENT: Connect to localhost as fallback
const getAPIBase = () => {
  // 1. Check explicit environment variable first
  if (process.env.REACT_APP_CROSSWORD_API_BASE) {
    return process.env.REACT_APP_CROSSWORD_API_BASE;
  }
  if (process.env.REACT_APP_API_BASE) {
    return process.env.REACT_APP_API_BASE;
  }

  // 2. If running on HTTPS or production domain, use production URL
  if (typeof window !== 'undefined' && window.location.protocol === 'https:' && window.location.hostname !== 'localhost') {
    return 'https://wisdomwarfare.onrender.com';
  }
...
const CrosswordGame = ({ user, gameCode, gameName, onLogout }) => {
  ...
  const syncCrosswordStateFromHttp = async () => {
    ...
    try {
      ...
      console.log(`📡 Grid fetch to ${API_BASE}/crossword/grid/${gameCode} status:`, gridRes.status);
      if (!gridRes.ok) {
        console.error('❌ Grid fetch failed:', gridRes.status, gridRes.statusText);
        setWaitingForTeacher(true);
      ...
      console.log('🧩 Grid data from HTTP:', gridData);
  ...
  useEffect(() => {
    ...
    newSocket.on('crosswordGrid', (data) => {
      ...
      console.log('🧩 Grid size:', data?.grid?.length, 'x', data?.grid?.[0]?.length);
      console.log('🧩 Total clues:', data?.clues?.length);
      if (!mountedRef.current) return;
      ...
      if (!hasPlayableGridPayload(data)) {
        console.warn('⚠️ Grid payload not playable:', data);
        gridLoadedRef.current = false;
        setWaitingForTeacher(true);
        setStatusMessage('Waiting for teacher to start the crossword');
        setLoading(false);
