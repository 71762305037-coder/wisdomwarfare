...
// Single API base for both quiz and crossword
const API_BASE = (
  ...
  const fetchGlobalLeaderboard = React.useCallback(async () => {
    ...
    try {
      const [wisdomRes, crosswordRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/leaderboard/crossword?limit=50`),
      ]);
...
function GamePage({ user, onLogout }) {
  ...
  const validateGameCode = useCallback(async (trimmedCode, gameType) => {
    // Both quiz and crossword use the same API_BASE endpoint
    ...
    let game = await tryValidate(API_BASE);
